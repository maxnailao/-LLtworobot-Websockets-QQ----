# 基于LLonerobot框架并用Websockets正向代理的QQ机器人--酪氨酸
...
qq = ''  # 机器人的qq账号
group_id = 114514# 机器人工作的群号
my_api_key = " "  # ai回复的API_KEY
my_base_url = "h"  # AI代理地址
max_message = 15 #机器人消息记忆储存数额，越高token消耗越快
robot_background = """ """#AI人设性格设置
...
async def main():  # 循环发送消息
    async with websockets.connect(uri) as websocket:
        ...
        while True:
            ...
            #处理群消息
            if receive_data.get('message_type') == 'group':
                #处理群一般消息
                if receive_data.get('post_type') == 'message':
                ...
                #处理点赞返回的消息
                if receive_data.get('message') == '今日同一好友点赞数已达上限':
                    echo = str(uuid.uuid4())
                    data = {
                        "action": "send_group_msg",
                        "params": {
                            "group_id": group_id,
                            "message": "不过今天已经给你点过赞了，不能再点了喵"
                        },
                        'echo': echo
                    }
                    await websocket.send(json.dumps(data))
